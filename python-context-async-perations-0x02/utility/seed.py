...
def get_config_parameters(file=None):
    ...
    values = [os.getenv('DB_USERNAME'), os.getenv('DB_HOST'), os.getenv('DB_PASSWORD'), os.getenv('DB_NAME')]
...
def connect_db(config=None):
    ...
    Returns:
        ...
        **updated to take the configuration parameters of the database connection. If not provided uses the get parameters method to 
        retrieve details from a secrets.env file
    """
    if not config:
        config = get_config_parameters()
    connection_pool = mysql.connector.pooling.MySQLConnectionPool(**config) # type: ignore
